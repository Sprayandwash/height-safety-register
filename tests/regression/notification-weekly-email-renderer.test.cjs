...
test('NOTIFY-WEEKLY-RENDER-001: weekly email drafting is authenticated and has no provider, scheduler, or delivery path', () => {
  assert.match(edgeFunction, /action === 'render_weekly_email'/);
  assert.match(edgeFunction, /delivery: 'disabled'/);
  assert.doesNotMatch(edgeFunction, /RESEND_API_KEY|api\.resend\.com|sendWeeklyEmail|cron\.schedule|operations_notification_deliveries.*insert/s);
});
