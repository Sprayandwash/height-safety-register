...
test('REG-058: Admin creates, blocks, unblocks, signs out and deletes a controlled private account',async({page})=>{
  ...
  await openAdmin(page);await page.getByText('Current Users',{exact:true}).click();const blockedRow=page.locator('.ops-user-row').filter({hasText:config.claimEmail});await acceptDialogDuring(page,()=>blockedRow.locator('[data-ops-account-unblock]').click());await expect(blockedRow).toContainText('Active',{timeout:15000});await signOut(page);await signIn(page,config.claimEmail,personalPassword);const vehicleCard=page.locator('.ops-home-vehicle');await expect(vehicleCard).toBeVisible({timeout:15000});await vehicleCard.click();await expect(page.locator('#opsShell h2')).toHaveText('Vehicle Checks');await signOut(page);
