...
| ID | Confirmed regression | Expected behaviour | Coverage target | Status |
| --- | --- | --- | --- | --- |
| REG-043 | User management was inaccessible or Edit user collapsed the Current Users section. | Admin can reach the canonical UI and edit a user without collapsing the working area. | Admin browser test | Automated static guard; dedicated Staging browser review implemented, awaiting the Admin test-account secrets and first run. |
...
| REG-046 | Height and Admin showed conflicting role definitions or duplicate management controls. | One canonical role source and user-management UI, in Admin only. | Role-matrix/browser test | Automated static guard; dedicated Staging Admin browser review implemented, awaiting first run. |
