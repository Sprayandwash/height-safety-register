...
function toggleRopeLengthField(){eqRopeLenWrap.classList.toggle("hidden",!typeNeedsLength(eqType.value));}
...
function startInspection(id,title="New Inspection"){if(!requirePerm(canInspect(),"Only Admin or Height equipment manager users can start inspections."))return;let e=equipment.find(x=>x.id===id);if(!e)return;inspectTitle.textContent=title;inSerial.value=e.serial;inType.value=e.type;inDate.value=today();inNextDue.value=addMonths(today(),6);inResult.value="Pass";renderChecklist();showTab("inspect");setTimeout(()=>inDate.focus(),250);}
...
async function saveInspection(){
  if(!requirePerm(canInspect(),"Only Admin or Height equipment manager users can save inspections."))return;
...
function selectNewEquipmentPhotos(files){if(!requirePerm(canAddPhotos(),"Only Admin or Height equipment manager users can add photos."))return;beginPhotoQueue(files,{mode:"pending"});}
function selectExistingEquipmentPhotos(equipmentId,files){if(!requirePerm(canAddPhotos(),"Only Admin or Height equipment manager users can add photos."))return;beginPhotoQueue(files,{mode:"existing",equipmentId});}
function selectInspectionPhotos(files){if(!requirePerm(canInspect(),"Only Admin or Height equipment manager users can add inspection photos."))return;beginPhotoQueue(files,{mode:"pendingInspection"});}
function selectExistingInspectionPhotos(inspectionId,equipmentId,files){if(!requirePerm(canInspect(),"Only Admin or Height equipment manager users can add inspection photos."))return;beginPhotoQueue(files,{mode:"existingInspection",inspectionId,equipmentId});}
